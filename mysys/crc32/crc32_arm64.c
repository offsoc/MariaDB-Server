...
static unsigned int crc32c_aarch64_pmull(unsigned, const void *, size_t);
# endif
...
#define PREF4X64L1(buffer, offset, itr)\
  __prefetch((const char *)buffer + (offset) + ((itr) + 0)*64);\
  __prefetch((const char *)buffer + (offset) + ((itr) + 1)*64);\
  __prefetch((const char *)buffer + (offset) + ((itr) + 2)*64);\
  __prefetch((const char *)buffer + (offset) + ((itr) + 3)*64);
#else
...
#ifdef HAVE_ARMV8_CRYPTO
static inline uint64_t poly64_mul(uint64_t a_scalar, uint64_t b_scalar)
{
#ifdef _MSC_VER
  __n64 a= vcreate_p64(a_scalar);
  __n64 b= vcreate_p64(b_scalar);

  __n128 res= neon_pmull_64(a, b);

  uint64x2_t res_u64= vreinterpretq_u64_p128(res);

  // For example, return the low 64 bits
  return vgetq_lane_u64(res_u64, 0);
#else /* __GNUC__ */
  return (uint64_t) vmull_p64(crc1, k2);
#endif
}
...
# ifdef HAVE_ARMV8_CRC_CRYPTO_INTRINSICS
  ...
  for (const poly64_t k1= 0xe417f38a, k2= 0x8f158014; (length-= 1024) >= 0; )
  {
     ...
     * crc0 multiply by K1
     */
    t1= poly64_mul(crc1, k2);
    t0= poly64_mul(crc0, k1);
