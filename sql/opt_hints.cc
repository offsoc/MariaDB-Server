...
bool Opt_hints_table::update_index_hint_maps(THD *thd, TABLE *tbl)
{
  ...
  tbl->force_index= is_force_index_hint(INDEX_HINT_ENUM);
  tbl->force_index_join=
      (tbl->force_index || is_force_index_hint(JOIN_INDEX_HINT_ENUM));
  tbl->force_index_group=
      (tbl->force_index || is_force_index_hint(GROUP_INDEX_HINT_ENUM));
  tbl->force_index_order=
      (tbl->force_index || is_force_index_hint(ORDER_INDEX_HINT_ENUM));

  if (tbl->force_index_join || tbl->force_index_group || tbl->force_index_order)
  {
